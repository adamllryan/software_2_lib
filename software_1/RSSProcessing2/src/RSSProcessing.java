...
public final class RSSProcessing {
    ...
    private static void processItem(XMLTree item, SimpleWriter out) {
        ...
        String title = (getChildElement(item,"title")==-1) ? item.child(getChildElement(item,"description")).child(0).label() : item.child(getChildElement(item,"title")).child(0).label();
        out.println("\nTitle: " + title);
        out.println("Link: " + item.child(getChildElement(item,"link")).child(0).label());
    }
