...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
    	if (n.compareTo(m) != 0) {
			if (n.compareTo(m)>0) 
				n.subtract(m);//if n is larger than m
			else
				m.subtract(n);
				reduceToGCD(n, m); //recursion until n=m
		} else m.clear();//make m default, =0
    }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (p.isZero()) {
	        ...
	        NaturalNumber base = n.newInstance();//copy of n to multiply later if p is odd or square if p is even
	        base.copyFrom(n);
	        NaturalNumber pTemp = p.newInstance();//copy of p so that p is restored
	        pTemp.copyFrom(p);
	        NaturalNumber nTemp = n.newInstance(); //variable to be manipulated
	        nTemp.copyFrom(n);
	        if (pTemp.compareTo(new NaturalNumber2(1))>0) {//while the copy of p > 1
		        if (isEven(pTemp)) {
		        	base.copyFrom(n);
		        	nTemp.multiply(base); //n*n
		        	pTemp.divide(new NaturalNumber2(2)); //p/2
		        	powerMod(nTemp,pTemp,m); //recursion with temp variables so transferfrom doesnt change n before all math has been done
		        } else {
		        	pTemp.decrement();//temp p - 1
		        	powerMod(nTemp,pTemp,m);
		        	nTemp.multiply(base);//multiply by base
		        }
	        }
	        n.transferFrom(nTemp.divide(m));//transfer remainder of the power function to n
        }
    }
     ...
     * </pre>
     */
    public static boolean isWitnessToCompositeness(NaturalNumber w, NaturalNumber n) {//values of n in the thousands quickly cause stack overflow errors
    	assert n.compareTo(new NaturalNumber2(2)) > 0 : "Violation of: n > 2";
        assert (new NaturalNumber2(1)).compareTo(w) < 0 : "Violation of: 1 < w";
        if (w.compareTo(n)==0) return false; //if they equal each other
        n.decrement();
        assert w.compareTo(n) < 0 : "Violation of: w < n - 1";
        n.increment();
        //start of code
        NaturalNumber temp1 = w.newInstance();//2 copies of w for manipulation
        temp1.copyFrom(w);
        NaturalNumber temp2 = w.newInstance();
        temp2.copyFrom(w);
        NaturalNumber nMin1 = new NaturalNumber2(n);
        powerMod(temp1, new NaturalNumber2(2), n);//w^2%n
        nMin1.decrement();//n-1
        powerMod(temp2, nMin1, n);//w^(n-1)%n
        //for the second part, you need the ! on the outside so that if it can't convert to int, it will automatically return true because 1 can be converted to int
        return (temp1.canConvertToInt() && temp1.toInt()==1) || !(temp2.canConvertToInt() && temp2.toInt()==1);
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
         ...
         */
        if (isEven(n) && n.compareTo(new NaturalNumber2(2))>0) return false;//if >2 and even then is divisible by 2 so return false 
        for (int i = 0; i<50; i++) {
        	NaturalNumber temp = randomNumber(n);
        	n.decrement();// turn to n-1 to check if fits requires
        	
        	if (temp.compareTo(n) < 0 && temp.compareTo(new NaturalNumber2(1))>0) {
        		n.increment();
        		if (isWitnessToCompositeness(temp, n))
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
         ...
         */
        //odd only because even numbers are multiples of 2
        
        int sentinel = 0;
        if (isEven(n)) n.decrement(); //if even pull down one so the +2 after will act as a +1
        while (sentinel==0) {
        	n.increment();//double increment i have no idea if this is more efficient than just n.add(new NaturalNumber2(2))
        	n.increment();
        	if (isPrime2(n)) sentinel = -1;
        }
    }
