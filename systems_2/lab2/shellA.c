...
int main(void)
{
	...
	/* Fill in the code for these steps:  
	 ...
	 (3) If bgrnd == 0, the parent will wait, 
        ...
        if ((pid = fork()) == -1) /*fork failed handler*/
        {
            printf("Fork failed");
        }
        else if (pid==0) /*child process executes command*/
        {
            if (execvp(iBuffer, args) == -1) {
                printf("Command failed to execute\n");
            }
        }
        else if (!bgrnd) /*parent with wait*/
        {
            do 
            {
                if ((pid = waitpid(pid, &status, WEXITSTATUS(status))) == -1) /*check for fail*/
                {
                    printf("Wait failed");
                }
            }
