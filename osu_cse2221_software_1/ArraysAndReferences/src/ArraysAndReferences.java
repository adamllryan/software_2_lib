...
public final class ArraysAndReferences {
    ...
    private static void computePartialProducts(NaturalNumber[] nnArray) {
        ...
        for (int i = 0; i<nnArray.length; i++) {
        	NaturalNumber[] temp = new NaturalNumber[i+1];
        	for (int j = 0; j<=i; j++) {
        		temp[j] = new NaturalNumber2();
        		temp[j].copyFrom(nnArray[j]);
        		//System.out.println(temp[j]);
        	}
        	System.out.println(productOfArrayElements(temp));
        }
