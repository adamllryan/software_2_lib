...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!r.atEOS()) {
	        ...
	        int pair = Integer.parseInt(line.substring(line.indexOf(',')+1));
	        System.out.println(key + ", " + pair);
	        priceMap.add(key, pair);
        }
        r.close();
    }
    ...
    private static int getOneOrder(SimpleReader input,
         ...
         */
        int total = sizePriceMap.value(input.nextLine());
        
        String line = input.nextLine();
        while (!(input.atEOS() || line.equals(""))) {
    ...
    private static void putPrice(SimpleWriter out, int price) {
        ...
        out.print(f.format(price/100.0));
		/*
		 * out.print("$" + price/100); if (price%100<10) out.print(".0" + price%100);
		 * else out.print("." + price%100);
		 */
