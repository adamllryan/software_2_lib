...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //takes str2 and cuts off the overlap then adds to str1
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        for (String s : temp) {
        	if (sentinel==0) {
        		if (s.indexOf(str) >-1)//if str is a substring of s in strSet
        			sentinel = -1;//sentinel will restore strSet
        		if (str.indexOf(s)==-1)
        			strSet.add(s);//if s is not a substring of str
        	}
        }
        if (sentinel==-1)//restore if str was found to be a substring
        	strSet.transferFrom(temp);
        else strSet.add(str); //if str is not a substring of any s in strSet, add
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        while (!input.atEOS()) {//i had to read the documentation to find this, what if input is read from System.in?
