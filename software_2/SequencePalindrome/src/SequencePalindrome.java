...
public final class SequencePalindrome {
    ...
    private static boolean isPalindrome(Sequence<Integer> s) {
        ...
        for (int i = 0; i<s.length()/2; i++) 
        	if (!s.entry(i).equals(s.entry(s.length()-i-1))) 
        		return false;
        	
        
        return true;
    }
    private static boolean isPalindromeRecursive(Sequence<Integer> s) {
        assert s != null : "Violation of: s is not null";
        ...
        if (s.length()==0 || s.length()==1)
        	...
        	Integer end = s.remove(s.length()-1);
        	if (!isPalindromeRecursive(s)) {
        		s.add(0, front);
        		s.add(s.length(), end);
        		return false;
        	} else if (front.equals(end)) {
        		s.add(0, front);
        		s.add(s.length(), end);
        		return true;
        	} else {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < sequences.length; i++) {
             ...
             */
            boolean correctResult = (isPalindromeRecursive(s) == results[i]);
