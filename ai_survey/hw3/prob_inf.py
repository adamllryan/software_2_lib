...
def calc_global_joint_prob(model, variableValues):
    ...
    for var in model.vars:
        val = read_cpt(model, var, variableValues)
        probability *= val if variableValues[var] else 1 - val
...
def calc_query_exact_brute(model, queryVar, queryVal, evidence):
    ...
    # This first attempt atrobabilistic inference will use the brute-force (table)
    ...
    for jptEntry in truefalse_combination_iterator(model.vars):
        ...
        if dict_issubset(jptEntry, evidence):
            pr_E += pr_entry
            if dict_issubset(jptEntry, {queryVar: queryVal}):
                pr_QE += pr_entry
...
def calc_query_exact_tree(model, queryVar, queryVal, evidence):
        ...
        print(
            ...
            + " ".join(
                (
                    ("sum(" + v + ")")
                    if m
                    else "P({0}|{1})".format(v, ",".join(model.varDist[v].parents))
                )
                for m, v in calcOrder
            )
        )
    ...
    prQ_T, prQ_F = recurse_calc_query_exact_tree(
        model, queryVar, evidence, variableValues, calcOrder
    )
    ...
    return prQ_T / (prQ_T + prQ_F) if queryVal else prQ_F / (prQ_T + prQ_F)


def recurse_calc_query_exact_tree(
    model, queryVar, evidence, variableValues, remainingCalc
):
    """
    ...
    if DEBUG_OUTPUT > 0:
        indent = "    " * (
            len(evidence) - sum(m for m, v in remainingCalc)
        )  # Indent based on how deep in the recursion we are
    ...
    marginalize, var = remainingCalc[
        0
    ]  # Grab details for the next term we have to deal with
    if marginalize:
        # Summation term, need to branch over all possible values and continue calculation
        if DEBUG_OUTPUT > 0:
            print(indent + "Sum over " + var)
        ...
        # calculate the probability of the query variable being true and false

        prQ_T, prQ_F = 0, 0
        for variableValues[var] in [True, False]:
            print(var, variableValues[var])
        ...
        if queryVar in model.varDist[var].parents:
            ...
            true = read_cpt(model, var, variableValues)
            variableValues[queryVar] = False
            false = read_cpt(model, var, variableValues)
            ...
            if DEBUG_OUTPUT > 0:
                print(
                    indent
                    + "P({0}|{1}) [QC]".format(
                        var, ",".join(model.varDist[var].parents)
                    ),
                    variableValues,
                    prQ_T,
                    prQ_F,
            ...
            """ T AND F SHOULD BE THE SAME (EMULATED CONSTANT) """
            prob = read_cpt(model, var, variableValues)
            prQ_T = prQ_F = prob if variableValues[var] else 1 - prob
            ...
            # print(variableValues[var], prQ_T, prQ_F)
...
def read_cpt(model, varName, condVals):
    ...
    if key not in varDist.cpt:
        raise IndexError(
            "CPT for variable '{0}' has no entry matching:\n{1}".format(
...
def truefalse_combination_iterator(entries):
    ...
    if len(entries) > 30:
        error(
            "truefalse_combination_iterator() does not support more than 30 entries at this time"
        )
...
def generate_exact_inf_term_seq(model, queryVar, evidence):
    ...
    def seq_from_hid_order(hOrd):
        ...
        prio.update(
            (v, -1) for v in model.vars if v not in prio
        )  # non-hidden variables get lowest prio so they don't count
        # Then rate each factor on the highest priority amongst its dependencies
        vOrd = [
            (
                max(chain((prio[v],), (prio[c] for c in model.varDist[v].parents))),
                True,
                v,
            )
            for v in model.vars
        ...
        vOrd.extend(
            (prio[h], False, h) for h in hOrd
        )  # Add placeholers for summations as well, the False ensures these will sort before their dependents
        vOrd.sort()
...
def read_model_file(filename):
    ...
    with open(filename, newline="") as csvfile:
        ...
        for row in ([e for e in x if len(e) > 0] for x in chain(csvreader, [[]])):
            ...
            if len(srow) == 0:
                ...
                if len(row) < varIdx + 1:
                    error(
                        "Malformat in csv line {0}: Too few columns for parent values and variable probability".format(
                            rowNum
                        )
                    )
                if len(parents) > 0:
                    key = frozenset(
                        zip(
                            parents,
                            (e.strip().upper().startswith("T") for e in row[0:-1]),
                        )
                    )
                else:
                    key = frozenset()
    ...
    for var in model.vars:  # Make sure every mentioned variable has an entry
        for p in model.varDist[var].parents:
            if p not in vCheck:
                error(
                    "Variable '{0}' has '{1}' as parent, but variable '{1}' was not defined".format(
                        var, p
                    )
                )
    for var in model.vars:  # Check every cpt for missing rows
        ...
        if missingCnt > 0:
            print(
                "Warning: read_model_file(): Variable '{0}' had {1} missing entries, filled with 0.5".format(
                    var, missingCnt
                )
            )
    # --------------------------------------------------------
    ...
    varsDep = [
        x for x in model.vars if len(model.varDist[x].parents) == 0
    ]  # Start from prior variables (no parents)
    idx = 0
    parentsLeft = {
        x: len(model.varDist[x].parents) for x in model.vars
    }  # Track how many of a node's parents are still not in the ordering
...
def main(args):
    ...
    if args.mode != "table" and args.mode != "print" and (args.query is None):
        ...
        if "=" not in args.query:
            error(
                "Query variable malformed, must follow VariableName=True or VariableName=False format"
            )
        s = args.query.split("=")
    ...
    if args.evidence is None:
        ...
        for e in args.evidence:
            if "=" not in e:
                error(
                    "Evidence argument '{0}' malformed, must follow VariableName=True or VariableName=False format".format(
                        e
                    )
                )
            s = e.split("=")
    ...
    if args.mode == "table":
        ...
        pr = None
        if args.mode == "brute":
            pr = calc_query_exact_brute(
                model, args.query[0], args.query[1], args.evidence
            )
        elif args.mode == "tree":
            pr = calc_query_exact_tree(
                model, args.query[0], args.query[1], args.evidence
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CSE3521 Homework 3 - Probabilistic Inference"
    )
    parser.add_argument(
        "--model",
        type=str,
        action="store",
        required=True,
        help="Input file to load model from",
    )
    parser.add_argument(
        "--mode",
        type=str,
        action="store",
        choices=["brute", "tree", "approx", "table", "print"],
        required=True,
        help="How to process the model",
    )
    parser.add_argument(
        "--query",
        "-q",
        type=str,
        action="store",
        help="Query variable to perform inference on, in format VariableName=True or VariableName=False",
    )
    parser.add_argument(
        "--evidence",
        "-e",
        type=str,
        action="append",
        help="Evidence variable and value, in format VariableName=True or VariableName=False\nRepeat argument for multiple variables",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        default=False,
        help="Enable debugging output statements",
    )
