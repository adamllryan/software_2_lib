    ...
    for i in range(N):
        for j in range(D):
            if j == 0:
                ...
                J[i, j] = p[0, 0] * np.log(X[i, 0]) * X[i, 0] ** p[1, 0]
            elif j == 2:
                J[i, j] = X[i, 0]
            elif j == 3:
                J[i, j] = 1
...
def nonlinear_regression_gn(X, Y, initialP):
    ...
    for iteration in range(GAUSS_NEWTON_ITERATIONS):  # PLEASE do not change this line
        j = calc_jacobian(X, p)
        ji = np.linalg.inv(np.matmul(np.transpose(j), j))
        p = p - np.matmul(np.matmul(ji, np.transpose(j)), model_function(X, p) - Y)
...
def nonlinear_regression_gd(X, Y, initialP):
    ...
    for iteration in range(
        ...
        p = p - LEARNING_RATE * 2 * np.matmul(
            np.transpose(calc_jacobian(X, p)), model_function(X, p) - Y
        )
...
def data_loader(args):
    ...
    if not osp.isfile(r"dataset_nonlinear.npz"):
        ...
        Y = a * (X**b) + c * X + d
        Y = Y + np.random.normal(scale=(np.max(Y) - np.min(Y)) * 0.02, size=N).reshape(
            -1, 1
        )
...
def auto_grade(display):
    ...
    J_expected = np.array(
        [
            8.538149682454624356e-01,
            -8.995838533071250087e-02,
            9.000000000000000222e-01,
            1.000000000000000000e00,
            1.153689732987166927e00,
            1.099583758894105007e-01,
    ...
    if not args.method.startswith("grad"):
        ...
        pExpected = np.array(
            [
                9.616951854092781193e-01,
                1.528478679323104217e00,
                1.039968168648498947e00,
                -1.001665398287315378e00,
            ]
        ).reshape(-1, 1)
    ...
    if not args.method.startswith("gauss"):
        ...
        pExpected = np.array(
            [
                8.729450285013207189e-01,
                1.557936080178939919e00,
                8.926387394411837706e-01,
                -1.088187586270846996e00,
            ]
        ).reshape(-1, 1)
...
def main(args):
    ...
    initialP = np.array([1, 1.5, 0, 0]).reshape(
        -1, 1
    )  # Cheating here, since I know how the data is generated
    if args.method.startswith("grad"):
