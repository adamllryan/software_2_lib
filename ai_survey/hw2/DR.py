...
def toy_data():
    ...
    X[2] = np.concatenate(
        (np.linspace(1.0, 2.0, num=int(m / 2)), np.linspace(2.0, 1.0, num=int(m / 2))),
        0,
    )
    return X, X[0]
    ...
    if args.data == "Swiss_Roll" or args.data == "toy_data":
        if new_X.shape[0] != 1:
            ...
            if args.method == "PCA" and args.save:
                ...
                np.savez(
                    "Results_" + args.data + "_" + str(args.out_dim) + ".npz",
                    mu=mu,
                    W=W,
                )
            plt.show()
            ...
            print(
                "The output dimensionality has to be larger than 1 for a scatter plot!"
            )
    elif args.data == "MNIST":
        if args.method == "PCA":
            xx = X[:, 0].reshape(-1, 1)
            ...
            print(
                "The first image is the mean image. the second to the last and the last are an"
        ...
        if args.method == "PCA" and args.save:
            np.savez(
                "Results_" + args.data + "_" + str(args.out_dim) + ".npz", mu=mu, W=W
            )
...
def auto_grade(mu, W):
    ...
    if W.ndim != 2:
        ...
        if W.shape[0] != 2 or W.shape[1] != 1:
            ...
            if (
                1.01
                > np.matmul(W.transpose(), np.array([[0.95782629], [0.28734789]]))
                > 0.99
                or -0.99
                > np.matmul(W.transpose(), np.array([[0.95782629], [0.28734789]]))
                > -1.01
...
def LE(X, out_dim, num_neighbor=5):
    ...
    Dis = (
        np.matmul(Dis.diagonal().reshape(-1, 1), np.ones((1, N)))
...
def PCA(X, out_dim):
    ...
    # 2. reshape to array with only one column
    # 3. save as mu
    mu = np.mean(X, axis=1).reshape(-1, 1)
    # print(mu)
    """([X-mu]*[X-mu]^T)/N"""
    # 1. multiply X-mu by its transpose
    # 2. divide by N
    # 3. save as Sigma
...
def main(args):
    ...
    X, phi = data_loader(
        args
    )  # X: the D-by-N data matrix (numpy array); phi: metadata of X (you can ignore it)
    ...
    if args.method == "PCA":
        ...
        mu, W = PCA(
            np.copy(X), out_dim
        )  # return mean and the projection matrix (numpy array)
        if args.data != "MNIST":
        ...
        """
        # 1. transpose W
        # 2. subtract mu from X
        # 3. multiply W transpose by X-mu
        # 4. save as new_X
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Running dimensionality reduction (DR)"
    )
